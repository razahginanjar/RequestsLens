...
```text
target/requestlens-agent-0.1.8-SNAPSHOT.jar
```
...
YAML config smoke variant:

```powershell
@'
http:
  port: 7099
  authToken: dev-token-123456789
trace:
  enabled: true
  packages: [demo]
  sampleRate: 1
...
persistence:
  enabled: false
'@ | Set-Content -Encoding UTF8 .\requestlens-agent.yaml

java "-javaagent:target/requestlens-agent-0.1.8-SNAPSHOT.jar=config=.\requestlens-agent.yaml" -jar demo/target/profiler-demo-app.jar --server.port=8080
```

When using the YAML variant, `/profiler/status` should show
...
curl -H "Authorization: Bearer $token" "http://127.0.0.1:7099/profiler/jfr/events?limit=100"
...
curl -H "Authorization: Bearer $token" "http://127.0.0.1:7099/profiler/async/status"
```
...
- `/profiler/status` shows `jfrConfigured: true`, `jfrAvailable: true`,
  `jfrRunning: true`, `jfrMaxEvents: 1000`, and `jfrThresholdMs: 0`.
- `/profiler/status` shows `asyncProfilerConfigured: true`,
  `asyncProfilerEmbedded: true`, and either `asyncProfilerAvailable: true` on
  supported Linux/macOS JVMs or an explicit unavailable error on unsupported
...
- `/profiler/jfr/events` returns `resource: jfr.events`, `configured: true`,
  `running: true`, `categories`, and bounded `events` after slow/CPU traffic.
...
- `/profiler/async/status` returns `resource: async.status`. On supported
  platforms, use dashboard Start/Stop or POST `/profiler/async/start` and
  `/profiler/async/stop`, then check `/profiler/async/collapsed`.
...
```powershell
$token = "dev-token-123456789"
java "-javaagent:target/requestlens-agent-0.1.8-SNAPSHOT.jar=port=7099,auth.token=$token,trace.enabled=true,trace.packages=demo,trace.sample.rate=1,profiler.persistence.enabled=true,profiler.persistence.path=target/smoke-profiler.db" -jar demo/target/profiler-demo-app.jar --server.port=8080
```
...
```powershell
java "-javaagent:target/requestlens-agent-0.1.8-SNAPSHOT.jar=port=7100,auth.token=$token,trace.enabled=true,trace.packages=demo,trace.sample.rate=1" -jar demo/target/profiler-demo-app.jar --server.port=8080
```
...
```powershell
java "-javaagent:target/requestlens-agent-0.1.8-SNAPSHOT.jar=port=7099,auth.token=$token,trace.enabled=true,trace.packages=demo,trace.sample.rate=1" -jar demo/target/profiler-demo-app.jar --server.port=8081
```
