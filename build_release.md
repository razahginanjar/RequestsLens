...
Current project milestone: `v0.1.8`. The Maven artifact version is controlled
by `pom.xml` and is `0.1.8-SNAPSHOT` for the current development build. For a
published `v0.1.8` release, set it to `0.1.8` before tagging.
...
```powershell
.\scripts\prepare-release.ps1 -ExpectedVersion 0.1.8-SNAPSHOT
```
